...
def matched_ip_and_port(pkt_to_match, match):
     # Compares the packet's port(s) against the match's port(s) 
    if pkt_to_match.tcp_in_pkt or pkt_to_match.udp_in_pkt:
        if not __matched_ports(pkt_to_match.header["dport"], match.header_fields["dport"]):
            return False
        
        if not __matched_ports(pkt_to_match.header["sport"], match.header_fields["sport"]):
            return False

    # Compares the packet's IP(s) against the match's IP(s) 
    if not __matched_IP(pkt_to_match.header["dst_ip"], match.header_fields["dst_ip"]):
        return False

    if not __matched_IP(pkt_to_match.header["src_ip"], match.header_fields["src_ip"]):
        return False

    return True

# Compares the header fields of a packet against the ones for a match
def matched_header_fields(pkt_to_match, match):
    if not __matched_IP_fields(pkt_to_match.header, match.header_fields):
        return False

    if pkt_to_match.tcp_in_pkt and not __matched_TCP_fields(pkt_to_match.header, match.header_fields):
        return False
    
    if pkt_to_match.icmp_in_pkt and not __matched_ICMP_fields(pkt_to_match.header, match.header_fields):
        return False
...
def __matched_IP_fields(pkt_fields, match_pkt_header):
    if "ip_proto" in match_pkt_header and not __matched_ip_proto(pkt_fields["ip_proto"], match_pkt_header["ip_proto"]["data"], match_pkt_header["ip_proto"]["comparator"]):
        return False
        
    if "ttl" in match_pkt_header and not compare_field(pkt_fields["ttl"], match_pkt_header["ttl"]["data"], match_pkt_header["ttl"]["comparator"]):
        return False

    if "id" in match_pkt_header and not compare_field(pkt_fields["id"], match_pkt_header["id"]["data"], match_pkt_header["id"]["comparator"]):
        return False
    
    if "ipopts" in match_pkt_header and not __matched_ipopts(pkt_fields["ipopts"], match_pkt_header["ipopts"]):
        return False

    if "fragbits" in match_pkt_header and not __matched_fragbits(pkt_fields["fragbits"], match_pkt_header["fragbits"]["data"], match_pkt_header["fragbits"]["comparator"]):
        return False

    return True

# Compares a packet's TCP fields against the TCP fields of a match 
def __matched_TCP_fields(pkt_fields, match_pkt_header):
    if "flags" in match_pkt_header and not __matched_tcp_flags(pkt_fields["flags"], match_pkt_header["flags"]):
        return False

    if "seq" in match_pkt_header and not compare_field(pkt_fields["seq"], match_pkt_header["seq"]["data"], match_pkt_header["seq"]["comparator"]):
        return False

    if "ack" in match_pkt_header and not compare_field(pkt_fields["ack"], match_pkt_header["ack"]["data"], match_pkt_header["ack"]["comparator"]):
        return False

    if "window" in match_pkt_header and not compare_field(pkt_fields["window"], match_pkt_header["window"]["data"], match_pkt_header["window"]["comparator"]):
        return False

    return True

# Compares a packet's ICMP fields against the ICMP fields of a match 
def __matched_ICMP_fields(pkt_fields, match_pkt_header):
    if "itype" in match_pkt_header and not compare_field(pkt_fields["itype"], match_pkt_header["itype"]["data"], match_pkt_header["itype"]["comparator"]):
        return False

    if "icode" in match_pkt_header and not compare_field(pkt_fields["icode"], match_pkt_header["icode"]["data"], match_pkt_header["icode"]["comparator"]):
        return False

    if "icmp_id" in match_pkt_header and not compare_field(pkt_fields["icmp_id"], match_pkt_header["icmp_id"]["data"], match_pkt_header["icmp_id"]["comparator"]):
        return False

    if "icmp_seq" in match_pkt_header and not compare_field(pkt_fields["icmp_seq"], match_pkt_header["icmp_seq"]["data"], match_pkt_header["icmp_seq"]["comparator"]):
        return False
    return True
...
def __matched_ipopts(pkt_ipopts, match_ipopts):
    ...
    if match_ipopts == "any" and pkt_ipopts_name in possible_ipopts:
        return True
    elif pkt_ipopts_name in possible_ipopts and possible_ipopts[pkt_ipopts_name] == match_ipopts:
        return True
...
def __matched_ip_proto(pkt_ip_proto, match_ip_proto, match_comparator):
    ops = { "":  pkt_ip_proto == int(match_ip_proto),
           "<":  pkt_ip_proto < int(match_ip_proto),
           ">":  pkt_ip_proto > int(match_ip_proto),
           "!":  pkt_ip_proto != int(match_ip_proto)}
